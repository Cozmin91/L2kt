...
public class QueenAnt extends L2AttackableAIScript
{
	...
	public String onExitZone(Creature character, ZoneType zone)
	{
		if (character instanceof GrandBoss)
		{
			...
			if (queen.getNpcId() == QUEEN)
				queen.teleToLocation(-21610, 181594, -5734, 0);
		}
		...
		switch (npc.getNpcId())
		{
			...
			case NURSE:
				...
				else if (caller.getNpcId() == QUEEN)
				{
					...
					{
						npc.setTarget(caller);
						((Attackable) npc).useMagic(SkillTable.FrequentSkill.NURSE_HEAL_1.getSkill());
					}
				}
				break;
		}
	...
	{
		final Playable realAttacker = (isPet && caster.getPet() != null) ? caster.getPet() : caster;
