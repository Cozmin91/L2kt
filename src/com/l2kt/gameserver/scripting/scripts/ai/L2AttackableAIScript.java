...
public class L2AttackableAIScript extends Quest
{
	...
	protected void registerNpcs()
	{
		...
		for (NpcTemplate template : NpcData.getInstance().getAllNpcs())
		{
			try
			{
				if (Attackable.class.isAssignableFrom(Class.forName("com.l2kt.gameserver.model.actor.instance." + template.getType())))
				{
	...
	public String onSkillSee(Npc npc, Player caster, L2Skill skill, WorldObject[] targets, boolean isPet)
	{
		...
		{
			if (attackable.hasAI() && (attackable.getAI().getDesire().getIntention() == CtrlIntention.ATTACK))
			{
	...
	public String onFactionCall(Npc npc, Npc caller, Player attacker, boolean isPet)
	{
		...
		if (attackable.getAI().getDesire().getIntention() != CtrlIntention.ATTACK)
		{
	...
	{
		npc.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED, attacker);
	...
	protected static void attack(Attackable npc, Creature victim, int aggro)
	{
		...
		npc.getAI().setIntention(CtrlIntention.ATTACK, victim);
	}
	...
	protected static boolean testCursesOnAttack(Npc npc, Creature attacker, int npcId)
	{
		...
		{
			final L2Skill curse = SkillTable.FrequentSkill.RAID_CURSE2.getSkill();
		...
		{
			final L2Skill curse = SkillTable.FrequentSkill.RAID_ANTI_STRIDER_SLOW.getSkill();
			if (attacker.getFirstEffect(curse) == null)
			{
				npc.broadcastPacket(new MagicSkillUse(npc, attacker, curse.getId(), curse.getLevel(), 300, 0));
				curse.getEffects(npc, attacker);
