package com.l2kt.gameserver.scripting.quests

import com.l2kt.gameserver.model.actor.Creature
import com.l2kt.gameserver.model.actor.Npc
import com.l2kt.gameserver.model.actor.instance.Player
import com.l2kt.gameserver.model.base.ClassId
import com.l2kt.gameserver.network.serverpackets.SocialAction
import com.l2kt.gameserver.scripting.Quest
import com.l2kt.gameserver.scripting.QuestState

class Q405_PathToACleric : Quest(405, "Path to a Cleric") {
    init {

        setItemsIds(
            LETTER_OF_ORDER_1,
            BOOK_OF_SIMPLON,
            BOOK_OF_PRAGA,
            BOOK_OF_VIVYAN,
            NECKLACE_OF_MOTHER,
            PENDANT_OF_MOTHER,
            LETTER_OF_ORDER_2,
            LIONEL_BOOK,
            CERTIFICATE_OF_GALLINT,
            LIONEL_COVENANT
        )

        addStartNpc(ZIGAUNT)
        addTalkId(ZIGAUNT, SIMPLON, PRAGA, VIVYAN, LIONEL, GALLINT)

        addKillId(20029, 20026)
    }

    override fun onAdvEvent(event: String, npc: Npc?, player: Player?): String? {
        val st = player?.getQuestState(qn) ?: return event

        if (event.equals("30022-05.htm", ignoreCase = true)) {
            st.state = Quest.STATE_STARTED
            st["cond"] = "1"
            st.playSound(QuestState.SOUND_ACCEPT)
            st.giveItems(LETTER_OF_ORDER_1, 1)
        }

        return event
    }

    override fun onTalk(npc: Npc, player: Player): String? {
        var htmltext = Quest.noQuestMsg
        val st = player.getQuestState(qn) ?: return htmltext

        when (st.state) {
            Quest.STATE_CREATED -> if (player.classId != ClassId.HUMAN_MYSTIC)
                htmltext = if (player.classId == ClassId.CLERIC) "30022-02a.htm" else "30022-02.htm"
            else if (player.level < 19)
                htmltext = "30022-03.htm"
            else if (st.hasQuestItems(MARK_OF_FATE))
                htmltext = "30022-04.htm"
            else
                htmltext = "30022-01.htm"

            Quest.STATE_STARTED -> {
                val cond = st.getInt("cond")
                when (npc.npcId) {
                    ZIGAUNT -> if (cond == 1)
                        htmltext = "30022-06.htm"
                    else if (cond == 2) {
                        htmltext = "30022-08.htm"
                        st["cond"] = "3"
                        st.playSound(QuestState.SOUND_MIDDLE)
                        st.takeItems(BOOK_OF_PRAGA, 1)
                        st.takeItems(BOOK_OF_VIVYAN, 1)
                        st.takeItems(BOOK_OF_SIMPLON, 3)
                        st.takeItems(LETTER_OF_ORDER_1, 1)
                        st.giveItems(LETTER_OF_ORDER_2, 1)
                    } else if (cond > 2 && cond < 6)
                        htmltext = "30022-07.htm"
                    else if (cond == 6) {
                        htmltext = "30022-09.htm"
                        st.takeItems(LETTER_OF_ORDER_2, 1)
                        st.takeItems(LIONEL_COVENANT, 1)
                        st.giveItems(MARK_OF_FATE, 1)
                        st.rewardExpAndSp(3200, 5610)
                        player.broadcastPacket(SocialAction(player, 3))
                        st.playSound(QuestState.SOUND_FINISH)
                        st.exitQuest(true)
                    }

                    SIMPLON -> if (cond == 1 && !st.hasQuestItems(BOOK_OF_SIMPLON)) {
                        htmltext = "30253-01.htm"
                        st.playSound(QuestState.SOUND_ITEMGET)
                        st.giveItems(BOOK_OF_SIMPLON, 3)
                    } else if (cond > 1 || st.hasQuestItems(BOOK_OF_SIMPLON))
                        htmltext = "30253-02.htm"

                    PRAGA -> if (cond == 1) {
                        if (!st.hasQuestItems(BOOK_OF_PRAGA) && !st.hasQuestItems(NECKLACE_OF_MOTHER) && st.hasQuestItems(
                                BOOK_OF_SIMPLON
                            )
                        ) {
                            htmltext = "30333-01.htm"
                            st.playSound(QuestState.SOUND_ITEMGET)
                            st.giveItems(NECKLACE_OF_MOTHER, 1)
                        } else if (!st.hasQuestItems(PENDANT_OF_MOTHER))
                            htmltext = "30333-02.htm"
                        else if (st.hasQuestItems(PENDANT_OF_MOTHER)) {
                            htmltext = "30333-03.htm"
                            st.takeItems(NECKLACE_OF_MOTHER, 1)
                            st.takeItems(PENDANT_OF_MOTHER, 1)
                            st.giveItems(BOOK_OF_PRAGA, 1)

                            if (st.hasQuestItems(BOOK_OF_VIVYAN)) {
                                st["cond"] = "2"
                                st.playSound(QuestState.SOUND_MIDDLE)
                            } else
                                st.playSound(QuestState.SOUND_ITEMGET)
                        }
                    } else if (cond > 1 || st.hasQuestItems(BOOK_OF_PRAGA))
                        htmltext = "30333-04.htm"

                    VIVYAN -> if (cond == 1 && !st.hasQuestItems(BOOK_OF_VIVYAN) && st.hasQuestItems(BOOK_OF_SIMPLON)) {
                        htmltext = "30030-01.htm"
                        st.giveItems(BOOK_OF_VIVYAN, 1)

                        if (st.hasQuestItems(BOOK_OF_PRAGA)) {
                            st["cond"] = "2"
                            st.playSound(QuestState.SOUND_MIDDLE)
                        } else
                            st.playSound(QuestState.SOUND_ITEMGET)
                    } else if (cond > 1 || st.hasQuestItems(BOOK_OF_VIVYAN))
                        htmltext = "30030-02.htm"

                    LIONEL -> if (cond < 3)
                        htmltext = "30408-02.htm"
                    else if (cond == 3) {
                        htmltext = "30408-01.htm"
                        st["cond"] = "4"
                        st.playSound(QuestState.SOUND_MIDDLE)
                        st.giveItems(LIONEL_BOOK, 1)
                    } else if (cond == 4)
                        htmltext = "30408-03.htm"
                    else if (cond == 5) {
                        htmltext = "30408-04.htm"
                        st["cond"] = "6"
                        st.playSound(QuestState.SOUND_MIDDLE)
                        st.takeItems(CERTIFICATE_OF_GALLINT, 1)
                        st.giveItems(LIONEL_COVENANT, 1)
                    } else if (cond == 6)
                        htmltext = "30408-05.htm"

                    GALLINT -> if (cond == 4) {
                        htmltext = "30017-01.htm"
                        st["cond"] = "5"
                        st.playSound(QuestState.SOUND_MIDDLE)
                        st.takeItems(LIONEL_BOOK, 1)
                        st.giveItems(CERTIFICATE_OF_GALLINT, 1)
                    } else if (cond > 4)
                        htmltext = "30017-02.htm"
                }
            }
        }

        return htmltext
    }

    override fun onKill(npc: Npc, killer: Creature?): String? {
        val player = killer?.actingPlayer

        val st = checkPlayerCondition(player, npc, "cond", "1") ?: return null

        if (st.hasQuestItems(NECKLACE_OF_MOTHER) && !st.hasQuestItems(PENDANT_OF_MOTHER)) {
            st.playSound(QuestState.SOUND_MIDDLE)
            st.giveItems(PENDANT_OF_MOTHER, 1)
        }

        return null
    }

    companion object {
        private val qn = "Q405_PathToACleric"

        // Items
        private val LETTER_OF_ORDER_1 = 1191
        private val LETTER_OF_ORDER_2 = 1192
        private val LIONEL_BOOK = 1193
        private val BOOK_OF_VIVYAN = 1194
        private val BOOK_OF_SIMPLON = 1195
        private val BOOK_OF_PRAGA = 1196
        private val CERTIFICATE_OF_GALLINT = 1197
        private val PENDANT_OF_MOTHER = 1198
        private val NECKLACE_OF_MOTHER = 1199
        private val LIONEL_COVENANT = 1200

        // NPCs
        private val GALLINT = 30017
        private val ZIGAUNT = 30022
        private val VIVYAN = 30030
        private val PRAGA = 30333
        private val SIMPLON = 30253
        private val LIONEL = 30408

        // Reward
        private val MARK_OF_FATE = 1201
    }
}