	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		if (command.startsWith("admin_polymorph"))
		{
			try
			{
				...
				WorldObject.PolyType info = WorldObject.PolyType.NPC;
				if (st.countTokens() > 1)
					info = Enum.valueOf(WorldObject.PolyType.class, st.nextToken().toUpperCase());
		...
		{
			if (target.getPolyType() == WorldObject.PolyType.DEFAULT)
			{
