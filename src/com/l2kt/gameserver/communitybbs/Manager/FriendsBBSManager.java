...
public class FriendsBBSManager extends BaseBBSManager
{
	...
	private static void showFriendsList(Player player, boolean delMsg)
	{
		...
		for (Integer id : list)
		{
			...
			StringUtil.INSTANCE.append(sb, "<a action=\"bypass _friend;select;", id, "\">[Select]</a>&nbsp;", friendName, " ", ((friend != null && friend.isOnline()) ? "(on)" : "(off)"), "<br1>");
		}
		...
		for (Integer id : selectedList)
		{
			...
			final Player friend = World.getInstance().getPlayer(id);
			StringUtil.INSTANCE.append(sb, "<a action=\"bypass _friend;deselect;", id, "\">[Deselect]</a>&nbsp;", friendName, " ", ((friend != null && friend.isOnline()) ? "(on)" : "(off)"), "<br1>");
		}
	...
	private static void showBlockList(Player player, boolean delMsg)
	{
		...
		for (Integer id : list)
		{
			...
			StringUtil.INSTANCE.append(sb, "<a action=\"bypass _block;select;", id, "\">[Select]</a>&nbsp;", blockName, " ", ((block != null && block.isOnline()) ? "(on)" : "(off)"), "<br1>");
		}
		...
		for (Integer id : selectedList)
		{
			...
			final Player block = World.getInstance().getPlayer(id);
			StringUtil.INSTANCE.append(sb, "<a action=\"bypass _block;deselect;", id, "\">[Deselect]</a>&nbsp;", blockName, " ", ((block != null && block.isOnline()) ? "(on)" : "(off)"), "<br1>");
		}
