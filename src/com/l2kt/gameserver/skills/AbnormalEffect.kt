 ...
 * @author DrHouse
 */
enum class AbnormalEffect constructor(val effectName: String, val mask: Int) {
    ...
    companion object {
        ...
        fun getByName(name: String): AbnormalEffect {
            for (eff in AbnormalEffect.values()) {
                if (eff.effectName == name)
                    return eff
            }
            throw NoSuchElementException("AbnormalEffect not found for name: '" + name + "'.\n Please check " + AbnormalEffect::class.java.canonicalName)
        }
    }
}