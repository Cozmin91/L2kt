...
public abstract class Inventory extends ItemContainer
{
	...
	public void equipItem(ItemInstance item)
	{
		if (getOwner() instanceof Player)
		{
			...
			if (((Player) getOwner()).isInStoreMode() || (item.isHeroItem() && !Hero.getInstance().isActiveHero(getOwnerId())))
				return;
		}
             ...
             PreparedStatement ps = con.prepareStatement(RESTORE_INVENTORY))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					if (getOwner() instanceof Player && item.isHeroItem() && !Hero.getInstance().isActiveHero(getOwnerId()))
