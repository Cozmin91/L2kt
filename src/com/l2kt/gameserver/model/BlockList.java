...
public class BlockList
{
	...
	public static void addToBlockList(Player listOwner, int targetId)
	{
		...
		{
			SystemMessage sm = SystemMessage.Companion.getSystemMessage(SystemMessageId.S1_ALREADY_IN_FRIENDS_LIST);
		...
		SystemMessage sm = SystemMessage.Companion.getSystemMessage(SystemMessageId.S1_WAS_ADDED_TO_YOUR_IGNORE_LIST);
		sm.addString(charName);
		listOwner.sendPacket(sm);
		...
		if (player != null)
		{
			sm = SystemMessage.Companion.getSystemMessage(SystemMessageId.S1_HAS_ADDED_YOU_TO_IGNORE_LIST);
	...
	public static void removeFromBlockList(Player listOwner, int targetId)
	{
		...
		if (!listOwner.getBlockList().getBlockList().contains(targetId))
		{
			sm = SystemMessage.Companion.getSystemMessage(SystemMessageId.TARGET_IS_INCORRECT);
			listOwner.sendPacket(sm);
			return;
		}
		...
		sm = SystemMessage.Companion.getSystemMessage(SystemMessageId.S1_WAS_REMOVED_FROM_YOUR_IGNORE_LIST);
		sm.addString(charName);
		listOwner.sendPacket(sm);
	}
