...
class PlayerStat(activeChar: Player) : PlayableStat(activeChar) {
    ...
    val maxCp: Int
        get() {
            val `val` =
                calcStat(Stats.MAX_CP, activeChar.template.getBaseCpMax(activeChar.level), null, null).toInt()
            if (`val` != _oldMaxCp) {
                _oldMaxCp = `val`
    ...
    val moveSpeed: Float
        get() {
            var baseValue = (if (activeChar.isInsideZone(ZoneId.WATER)) baseSwimSpeed else baseMoveSpeed).toFloat()
