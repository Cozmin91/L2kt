...
public class AttackableAI extends CreatureAI implements Runnable
{
	...
	protected void onEvtAttacked(Creature attacker)
	{
		...
		if (!me.isCoreAIDisabled() && (_desire.getIntention() != CtrlIntention.ATTACK || me.getMostHated() != getTarget()))
		{
			me.setRunning();
			...
			if (actorClans != null && me.getAttackByList().contains(attacker))
			{
				for (Attackable called : me.getKnownTypeInRadius(Attackable.class, me.getTemplate().getClanRange()))
				{
					...
					if ((calledIntention == CtrlIntention.IDLE || calledIntention == CtrlIntention.ACTIVE || (calledIntention == CtrlIntention.MOVE_TO && !called.isRunning())) && GeoEngine.INSTANCE.canSeeTarget(me, called))
					{
						...
						{
							final List<Quest> scripts = called.getTemplate().getEventQuests(EventType.ON_FACTION_CALL);
							if (scripts != null)
							{
	...
	protected void onEvtAggression(Creature target, int aggro)
	{
		...
		// Set the Intention to ATTACK and make the character running, but only if the AI isn't disabled.
		if (!me.isCoreAIDisabled() && _desire.getIntention() != CtrlIntention.ATTACK)
		{
			me.setRunning();
