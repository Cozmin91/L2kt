...
class GMViewWarehouseWithdrawList : L2GameServerPacket {
    ...
    override fun writeImpl() {
        ...
        for (temp in _items) {
            ...
            writeD(temp.objectId)
            writeD(if (temp.isWeapon && temp.isAugmented) 0x0000FFFF and temp.augmentation.augmentationId else 0)
            writeD(if (temp.isWeapon && temp.isAugmented) temp.augmentation.augmentationId shr 16 else 0)
        }
    }
}