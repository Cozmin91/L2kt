...
public class PetInventoryUpdate extends L2GameServerPacket
{
	...
	{
		if (item != null)
			_items.add(new ItemInfo(item, ItemInstance.ItemState.ADDED));
	}
	
	public void addModifiedItem(ItemInstance item)
	{
		if (item != null)
			_items.add(new ItemInfo(item, ItemInstance.ItemState.MODIFIED));
	}
	
	public void addRemovedItem(ItemInstance item)
	{
		if (item != null)
			_items.add(new ItemInfo(item, ItemInstance.ItemState.REMOVED));
	}
