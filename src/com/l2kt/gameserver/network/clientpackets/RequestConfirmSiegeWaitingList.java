...
public final class RequestConfirmSiegeWaitingList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!castle.getSiege().isRegistrationOver())
		{
			if (_approved == 1)
			{
				if (castle.getSiege().checkSide(clan, Siege.SiegeSide.PENDING))
					castle.getSiege().registerClan(clan, Siege.SiegeSide.DEFENDER);
			}
			else
			{
				if (castle.getSiege().checkSides(clan, Siege.SiegeSide.PENDING, Siege.SiegeSide.DEFENDER))
