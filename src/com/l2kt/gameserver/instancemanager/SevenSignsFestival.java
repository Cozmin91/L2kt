...
public class SevenSignsFestival
{
	...
	protected void startFestivalManager()
	{
		...
		_managerScheduledTask = ThreadPool.INSTANCE.scheduleAtFixedRate(fm, Config.ALT_FESTIVAL_MANAGER_START, Config.ALT_FESTIVAL_CYCLE_LENGTH);
	...
	protected void restoreFestivalData()
	{
		try (Connection con = L2DatabaseFactory.INSTANCE.getConnection())
		{
			...
			while (rset.next())
			{
				...
				set.set("cabal", Enum.valueOf(SevenSigns.CabalType.class, cabal));
				set.set("cycle", festivalCycle);
				set.set("date", rset.getString("date"));
				set.set("score", rset.getInt("score"));
				set.set("members", rset.getString("members"));
		...
		try (Connection con = L2DatabaseFactory.INSTANCE.getConnection())
		{
			...
			for (Map<Integer, StatsSet> map : _festivalData.values())
			{
				for (StatsSet set : map.values())
				{
					...
					statementUpdate.setLong(1, Long.valueOf(set.getString("date")));
					...
					statementInsert.setLong(4, Long.valueOf(set.getString("date")));
	...
	protected void resetFestivalData(boolean updateSettings)
	{
		...
		for (int i = 0; i < FESTIVAL_COUNT * 2; i++)
		{
			...
			if (i >= FESTIVAL_COUNT)
				set.set("cabal", SevenSigns.CabalType.DAWN);
			else
				set.set("cabal", SevenSigns.CabalType.DUSK);
	...
	public final List<Integer> getParticipants(SevenSigns.CabalType oracle, int festivalId)
	{
	...
	public final List<Integer> getPreviousParticipants(SevenSigns.CabalType oracle, int festivalId)
	{
		if (oracle == SevenSigns.CabalType.DAWN)
	...
	public void setParticipants(SevenSigns.CabalType oracle, int festivalId, Party festivalParty)
	{
	...
	public void updateParticipants(Player player, Party festivalParty)
	{
		...
		final SevenSigns.CabalType oracle = SevenSigns.CabalType.VALUES[playerFestInfo[0]];
	...
	public final int getFinalScore(SevenSigns.CabalType oracle, int festivalId)
	{
		if (oracle == SevenSigns.CabalType.DAWN)
	...
	public final int getHighestScore(SevenSigns.CabalType oracle, int festivalId)
	{
	 ...
	 */
	public final StatsSet getHighestScoreData(SevenSigns.CabalType oracle, int festivalId)
	{
	 ...
	 */
	public boolean setFinalScore(Player player, SevenSigns.CabalType oracle, FestivalType festival, int offeringScore)
	{
		final int festivalId = festival.ordinal();
		
		int currDawnHighScore = getHighestScore(SevenSigns.CabalType.DAWN, festivalId);
		int currDuskHighScore = getHighestScore(SevenSigns.CabalType.DUSK, festivalId);
		...
		if (offeringScore > thisCabalHighScore)
		{
			...
			if (offeringScore > otherCabalHighScore)
			{
				...
				SevenSigns.getInstance().addFestivalScore(oracle, festival.getMaxScore());
				
				_log.info("SevenSignsFestival: This is the highest score overall so far for the " + festival.getName() + " festival!");
			}
	 ...
	 */
	public final boolean increaseChallenge(SevenSigns.CabalType oracle, int festivalId)
	{
	...
	private class FestivalManager implements Runnable
	{
			...
			try
			{
				...
				for (int i = 0; i < FESTIVAL_COUNT; i++)
				{
					if (_duskFestivalParticipants.get(i) != null)
						_festivalInstances.put(10 + i, new L2DarknessFestival(SevenSigns.CabalType.DUSK, i));
					
					if (_dawnFestivalParticipants.get(i) != null)
						_festivalInstances.put(20 + i, new L2DarknessFestival(SevenSigns.CabalType.DAWN, i));
				}
		 ...
		 */
		public final L2DarknessFestival getFestivalInstance(SevenSigns.CabalType oracle, int festivalId)
		{
			...
			festivalId += (oracle == SevenSigns.CabalType.DUSK) ? 10 : 20;
	...
	{
		protected final SevenSigns.CabalType _cabal;
		...
		protected L2DarknessFestival(SevenSigns.CabalType cabal, int levelRange)
		{
			_cabal = cabal;
			...
			if (cabal == SevenSigns.CabalType.DAWN)
			{
		...
		protected void moveMonstersToCenter()
		{
			for (FestivalMonster festivalMob : _npcInsts)
			{
				...
				CtrlIntention currIntention = festivalMob.getAI().getDesire().getIntention();
		...
		protected void spawnFestivalMonsters(int respawnDelay, int spawnType)
		{
			...
			switch (spawnType)
			{
				case 0:
				case 1:
					_npcSpawns = (_cabal == SevenSigns.CabalType.DAWN) ? FESTIVAL_DAWN_PRIMARY_SPAWNS[_levelRange] : FESTIVAL_DUSK_PRIMARY_SPAWNS[_levelRange];
					break;
				
				case 2:
					_npcSpawns = (_cabal == SevenSigns.CabalType.DAWN) ? FESTIVAL_DAWN_SECONDARY_SPAWNS[_levelRange] : FESTIVAL_DUSK_SECONDARY_SPAWNS[_levelRange];
					break;
				
				case 3:
					_npcSpawns = (_cabal == SevenSigns.CabalType.DAWN) ? FESTIVAL_DAWN_CHEST_SPAWNS[_levelRange] : FESTIVAL_DUSK_CHEST_SPAWNS[_levelRange];
					break;
			}
